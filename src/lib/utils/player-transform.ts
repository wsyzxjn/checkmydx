...
// maimai DX class rank (阶级) id -> label.
const CLASS_RANK_LABELS = ['白', '黄', '绿', '蓝', '紫', '红', '金', '虹'];

export function classRankLabel(rank: number | null): string | null {
	if (rank == null || rank < 0 || rank >= CLASS_RANK_LABELS.length) return null;
	return CLASS_RANK_LABELS[rank];
}
