...
export async function getProfileViews(options: {
  ...
  const result = await getViewCount(
    ...
    buildCacheKey(normalizedUsername),
  );
  ...
  try {
    ...
    if (shouldCountView(seen, normalizedUsername)) optimistic = 1;
  } catch {
...
export async function recordProfileView(options: {
  ...
  try {
    ...
    const { shouldCount } = handleDeduplication(cookies, normalizedUsername);
    ...
    if (caches) {
      await setCachedCount(caches, buildCacheKey(normalizedUsername), {
        count: next,
        cachedAt: Date.now(),
      });
    }
