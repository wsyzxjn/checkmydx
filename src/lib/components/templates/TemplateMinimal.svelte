<script lang="ts">
	...
	import { formatNumber, formatSyncDate, courseRankLabel, classRankLabel, totalSyncs } from '$lib/utils/player-transform';
...
<div class="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8 {className}">
	...
	<header class="mb-16 text-center">
		...
		{#if course || classRank || profile.identity.star != null}
			<div class="mt-4 flex flex-wrap items-center justify-center gap-2">
				{#if course}
					<Badge variant="solid" size="md" color="#3291FF">{course}</Badge>
				{/if}
				{#if classRank}
					<Badge variant="outline" size="md">{classRank}</Badge>
				{/if}
	...
	{#if profile.scores.length > 0}
		<section class="mb-16">
			<h2 class="mb-8 text-center text-xs font-semibold uppercase tracking-widest text-text-tertiary">代表成绩</h2>
			<div class="space-y-3">
