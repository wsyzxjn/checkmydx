...
<section class="relative z-10 flex flex-col items-center justify-center px-6 pb-20 pt-16 text-center md:pb-32 md:pt-40">
	<div class="animate-fade-in relative z-10 max-w-4xl">
		...
		<div class="glass-panel mx-auto flex w-full max-w-130 items-center gap-2 rounded-2xl p-2 transition-all focus-within:ring-2 focus-within:ring-saas-green">
			<div class="relative flex flex-1 items-center px-3">
				<svg class="h-5 w-5 text-text-tertiary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v18m9-9H3m15.36-6.36L5.64 18.36m12.72 0L5.64 5.64" />
				</svg>
				<input
					type="text"
		...
		{#if isLxnsOAuthConfigured()}
			<div class="mt-4">
				<Button variant="secondary" size="md" onclick={handleConnectLxns} class="h-10 rounded-xl px-6 font-semibold">
					连接 LXNS
				</Button>
			</div>
		{/if}
		...
		<p class="animate-fade-in-delay-1 mt-6 text-sm text-text-tertiary">
			试试示例玩家：
			<button type="button" onclick={() => setUsername('amatsuka')} class="ml-1 cursor-pointer font-medium text-text-secondary transition-colors hover:text-accent-green">amatsuka</button>,
			<button type="button" onclick={() => setUsername('rainbow14')} class="cursor-pointer font-medium text-text-secondary transition-colors hover:text-accent-green">rainbow14</button>,
			<button type="button" onclick={() => setUsername('dxstar')} class="cursor-pointer font-medium text-text-secondary transition-colors hover:text-accent-green">dxstar</button>
		</p>
	</div>
</section>
