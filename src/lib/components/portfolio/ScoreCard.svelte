...
<div class="group flex gap-4 overflow-hidden rounded-md border border-border-default bg-bg-secondary p-4 transition-all hover:border-border-subtle hover:bg-bg-tertiary {className}">
	<!-- Jacket -->
	...
	<div class="flex min-w-0 flex-1 flex-col">
		<div class="flex items-start justify-between gap-2">
			...
			{#if score.difficulty}
				<Badge variant="outline" size="sm" color={score.difficulty.includes('14+') ? '#ef4444' : score.difficulty.includes('14') ? '#f59e0b' : '#22c55e'} class="flex-shrink-0">
					{score.difficulty}
				</Badge>
			{/if}
		</div>
		...
		<div class="mt-auto flex items-center gap-4 pt-2 text-xs text-text-tertiary">
			...
			<span class="ml-auto rounded bg-bg-tertiary px-1.5 py-0.5 text-[10px] text-text-secondary">{typeLabel}</span>
		</div>
	</div>
</div>
